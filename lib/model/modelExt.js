module.exports = {
	...
	readAll: function(callback){
		this.srvr.db[this.name].find().toArray(
            function(err, data){
	...
	insert: function(data, callback){
		...
		this.srvr.db[this.name].insert(newItem, function(err, items){
			if (!err) {
	...
	update: function(data, callback){
		...
		this.srvr.db[self.name].updateById(_id, clone, options, function(err, items){
			callback(err, items)
		})
	...
	delete: function(callback){
		this.findAll({_dirty:'deleted'}).each(function(item){
			this.srvr.db[this.name].remove({_id: item._id}, function(err){
				callback(err)
			})
		})
	},
