module.exports = function(srv){
	...
	return {
		...
		getAll: function( tenant, callback ){
			var self = this
		...
		getById: function( tenant, id, callback ){
			var self = this
		...
		getByText: function( tenant, text, callback ){
			var self = this
		...
		getByQuery: function( tenant, query, callback ){
			var self = this
		...
		insert: function( tenant, record, callback ){
			...
			self.collection( tenant ).insert(newItem, function(err, items){
				if (!err) {
		...
		update: function( tenant, record, callback ){
			var self = this
			...
			function afterGet( err, item ){
				...
				self.collection( tenant ).updateById(_id, clone, options, function(err, items){
					callback(err, items)
				})
			}

			self.getById( tenant, record._id, afterGet )			

		},

		delete: function( tenant, callback ){
			this.getAll({_dirty:'deleted'}).each(function(item){
				self.collection( tenant ).remove({_id: item._id}, function(err){
					callback(err)
				})
			})
		},
		...
		options: function(tenant, query, afterList){
			var self = this
			  , options = []
			...
			if (query){
				self.getByQuery( tenant, query, afterRead )
			} else {
				self.getAll( tenant, afterRead )
			}
