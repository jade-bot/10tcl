(function($){
	...
	$.view = {
		...
		editLinkClick: function(id){
			...
			$('.list-row[data-id={1}]'.assign(id)).addClass('info')
			$('#crud-form').show('fast')
		},
		...
		selectPrev: function(){
			var selected = $('.list-row.info').attr('data-idx').toNumber()
			if (selected === 0) return
			selected--
			$('.info').removeClass('info')
			$('.list-row[data-idx={1}]'.assign(selected)).addClass('info')
		},

		selectNext: function(){
			var rows = $('.list-row').length - 1
			  , selected = $('.list-row.info').attr('data-idx').toNumber()
		...
		startThemeRoller: function(){
			$('.theme-option').click(function(ev){
				var title = $(this).attr('data-title')
				$('link[bootstrap]').each(function(){
					$(this).attr('rel', 'alt-stylesheet')
					this.disabled = true
				})
				$('link[title={1}]'.assign(title))[0].disabled = false
				$('link[title={1}]'.assign(title)).attr('rel', 'stylesheet')
			})
		}
	}
