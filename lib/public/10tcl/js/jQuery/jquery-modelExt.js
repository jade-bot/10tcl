	...
	$.model.merge({
		...
		insert: function(record, cb){
			function then(record){
				...
				$.view.applyTemplates()
				$.ambiance({message: 'Registro incluido.', timeout: 2, type: 'success'})
				if (cb) cb()
			}
		...
		update: function(record, cb){
			function then(record, txt, jqXHR){
				...
				$.view.applyTemplates()
				$.ambiance({message: 'Registro alterado.', timeout: 2, type: 'success'})
				if (cb) cb()
			}
		...
		errorHandler: function(jqXHR, txt, err){
			...
			errs.each(function(field, err){
				...
				$.ambiance({message: err, timeout: 6, type: 'error'})
				alertify.error(err)
			})
		},
